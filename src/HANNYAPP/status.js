	...
	return (
		<div className="wrapper">
			...
			<div style={{ marginLeft: 20, marginTop: 20 }}>
				...
				<div>
					completed{" "}
					{(data.allMessages.edges[data.allMessages.edges.length - 1]
						.node.lastIdx /
						938) *
						100}{" "}
					%
				...
				<div>
					<div>hanny</div>
					<div className={classes.extend}>
						{data.allMessages.edges.map(
							(row) =>
								row.node.user.username === "hanny" && (
									<div>{row.node.message}</div>
								)
						)}
					</div>
