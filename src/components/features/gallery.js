...
function Gallery() {
    ...
    function handleFiles(e){
        ...
        if (e.target.files[0]){
            setFileData({formData:{uploadType:'gallery',pk:uuid.v4(),
            content:e.target.files[0],title:textChange.title,description:textChange.description}})
        }
        else {
            alert("No files ")
        }
    ...
    //     e.preventDefault();
    //     if (textChange.title !=='' && textChange.description!==''){
    //         // addTexts(textChange);
    //         // setFileData({...fileData,title:textChange.title,description:textChange.description})
    //     }
    ...
    return (
        <div>
            <div style={{ backgroundColor: '#FFF',  width: 182, height: 300, borderRadius:12,border:"1px solid #FFF",fontSize:12}}>
