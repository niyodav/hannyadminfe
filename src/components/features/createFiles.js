...
function submitForm(contentType, data, setResponse) {
    ...
    function UploadFiles({addFile,category,uploadType,blockId}) {
        ...
        //         formData.append("inputType", String(uploadType));
        //         // formData.append("blockId", String(selectedBlock.id));


        //         if (uploadType==="gallery" && textChange){

        //             // console.log(textChange.title)
        ...
        function handleFiles(e){
            setFile({...file,uploadType:uploadType,pk:uuid.v4()
    ...
    return (
        <div>
            ...
            {/* <div style={{  width: 182, height: 100,textAlign: 'center' ,paddingTop:50,}}>
                 ...
                 {uploadType!="gallery"?
                 <div   style={{ backgroundColor: '#FFF',cursor:"pointer", width: 182, height: 182, textAlign: 'center' ,paddingTop:40, borderRadius:12,border:"1px solid #FFF",fontSize:12,display:img?'none':"block"}} onClick={(e)=>{setShowUpload("block")}} >
                    <input type="file" id="upload" hidden style={{display:"none"}}  onChange={handleFiles}/>
                    <label for="upload" style={{cursor:"pointer"}}><img src="https://hanny-uploads.s3.amazonaws.com/USER/UPLOADS/ic_camera.png"/></label>
                    <label for="upload" style={{cursor:"pointer"}}>Upload {uploadType}</label>
                    ...
                    <TextField
                            ...
                            type='text'
                            placeholder={"OR insert "+uploadType+ " url here "}
                            // value ={item.name}
                            // onBlur={handleOnBlur}
