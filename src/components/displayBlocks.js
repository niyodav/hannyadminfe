...
function DisplayBlocks({search,addNewEditedValue}) {
    ...
    useEffect(() => {
        ...
        return () =>{
            setNewCategory({
                ...newCategory,block:false
            });
            setUpdateCategory({
                ...updateCategory,block:false
            });
            // setEdit(false)
    ...
    },[newCategory.block,updateCategory.block,selectedMenu.block,selectedBlock.id]);

    const { loading, error, data,refetch } = useQuery(TS_BLOCKS,{
        variables:{scenerioId:selectedCategoryContext.scenerio?selectedCategoryContext.scenerio.id:fk,orderBy:"name"},
    });
...
function handleOnBlur(e){
    ...
    if(modifiedvalue.name){
        ...
        setSelectedMenu({...setSelectedMenu,block:{id:false,action:false}})
    }
...
// let block  = false
    ...
    console.log(selectedBlock,"fjhkhgkethk", block)
    // else if(data.tsBlocks.edges.length<=0){
    ...
    let prevScenerioeName = selectedCategoryContext.scenerio?selectedCategoryContext.scenerio:false
        ...
        <div>
            ...
            <div>
                ...
                <Link to={locationContext.challengeGroup} className={classes.upperLinks}>대화관리</Link>
                    ...
                    <Link to={prevChallengeGroupName?"/challenge/"+prevChallengeGroupName.id: locationContext.challenge} className={classes.upperLinks}>
                    ...
                    <img src={"https://hanny-uploads.s3.amazonaws.com/USER/UPLOADS/next.png"} alt="increment" style={{width:20,height:15,marginRight:5, marginLeft:5}}/>    
                    <Link to={prevScenerioeName?"/block/"+prevScenerioeName.id:locationContext.block}
            ...
            <div className={classes.blockContainer}  ref={outerRef}>
                ...
                
                {
                    ...
                    data.tsBlocks.edges.map(row=>(
                        selectedMenu.block.action&&selectedMenu.block.action==="edit"&&
                        selectedMenu.block.id===row.node.blockId ?
                        <form>
                        <TextField
                            placeholder={row.node.name}
                            className={classes.block}
                            onChange={(e)=>{handleOnChange(e,row.node.blockId)}}
                            value={modifiedvalue.id===row.node.id ? modifiedvalue.name : null}
                            onBlur={handleOnBlur}
                        ...
                        <Button className={classes.block}
                            ...
                            // onMouseDown={}
                            // ref={outerRef}
                            style={block.id===row.node.blockId?{background: "#ffebbaff"}:null}
                            >
...
{/* 
            ...
            {
            selectedMenu.block.action&&selectedMenu.block.action==="edit"&& updateCategory.block.name&&

            // updateCategory.block.name && edit && 
                <UpdateBlock data={{blockId:updateCategory.block.id,
                    name:updateCategory.block.name}}  />
            }

        {
            selectedMenu.block.action&&selectedMenu.block.action==="delete"&&

            // updateCategory.block.name && edit && 
       
                    <DeleteBlock data={{blockId:selectedMenu.block.id}}/>
