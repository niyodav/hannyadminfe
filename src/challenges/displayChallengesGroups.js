...
function DisplayChallengesGroups({ search }) {
	...
	useEffect(() => {
		...
		setLocationContext({
			...locationContext,
			challengeGroup: location.pathname,
		});
		return () => {
			setNewCategory({
				...newCategory,
		...
		<div className="wrapper">
			<div>
				<h1>
					...
					/>
					<Link to="/" className={classes.upperLinks}>
						대화관리
					</Link>
				</h1>
			</div>
			...
			<div className="display-table">
				<DisplayTable
					data={data.challengeGroups.edges}
					fields={{
						id: "challengeId",
						name: "name",
						title: "해피 챌린지 그룹명",
						nextTableTitle: "해피 챌린지 수",
						path: "challenge",
						nextTotalCount: "groupChallenge",
					}}
			...
			{updateCategory.challengeGroup && (
				<UpdateChallengeGroup
					data={{
						challengeId: updateCategory.challengeGroup.id,
						name: updateCategory.challengeGroup.content,
					}}
				/>
			)}
			{selectedMenu.challengeGroup.action &&
				selectedMenu.challengeGroup.action === "delete" && (
					<DeleteChallengeGroup
						data={{ challengeId: selectedMenu.challengeGroup.id }}
					/>
				)}
		</div>
	);
